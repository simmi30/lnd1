...
func setup(t *testing.T, initialChans []LocalChannel) (*testContext, func()) {
	...
	var availableFunds btcutil.Amount = 10 * btcutil.SatoshiPerBrocoin
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := LocalChannel{
		ChanID:  randChanID(),
		Balance: btcutil.SatoshiPerBrocoin,
	}
...
func TestAgentHeuristicUpdateSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     1 * btcutil.SatoshiPerBrocoin,
		},
	)
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * btcutil.SatoshiPerBrocoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []LocalChannel{
		...
		{
			ChanID:  randChanID(),
			Balance: btcutil.SatoshiPerBrocoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += btcutil.SatoshiPerBrocoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * btcutil.SatoshiPerBrocoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != btcutil.SatoshiPerBrocoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					btcutil.SatoshiPerBrocoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * btcutil.SatoshiPerBrocoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, btcutil.SatoshiPerBrocoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget btcutil.Amount = 5 * btcutil.SatoshiPerBrocoin
	...
	channelBudget = btcutil.SatoshiPerBrocoin * 3 / 2
	...
	// our budget.
	channelBudget = btcutil.SatoshiPerBrocoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,
